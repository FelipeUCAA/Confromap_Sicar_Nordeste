...
async function checkClipboardForCoordinates() {
    ...
    map.getView().fit([-6349194.450895, -2095812.942262, -3231866.898950, -107429.617646], map.getSize());
}
...
function downloadTXT(popupInfo) {
    ...
    const nomeArquivo = popupInfo.cod_imovel || "arquivo_dados";
...
function downloadKML(coordinates, nome, descricao, popupInfo) {
    const nomeArquivo = popupInfo.cod_imovel || "arquivo_kml";
...
function downloadGeoJSON(coordinates, nome, descricao, popupInfo) {
    const nomeArquivo = popupInfo.cod_imovel || "arquivo_geojson";
...
async function downloadShapefileHTTP(coordinates, nome, descricao, popupInfo) {
    const nomeArquivo = popupInfo.cod_imovel || "arquivo_shapefile";
